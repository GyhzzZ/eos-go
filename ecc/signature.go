...
import (
	...
	"github.com/GyhzzZ/eos-go/btcsuite/btcd/btcec"
	"github.com/GyhzzZ/eos-go/btcsuite/btcutil/base58"
)
...
func (s Signature) Verify(hash []byte, pubKey PublicKey) bool {
	if s.Curve != CurveK1 {
		fmt.Println("WARN: github.com/GyhzzZ/eos-go/ecc library does not support the R1 curve yet")
		return false
	}
...
func (s Signature) PublicKey(hash []byte) (out PublicKey, err error) {
	if s.Curve != CurveK1 {
		return out, fmt.Errorf("WARN: github.com/GyhzzZ/eos-go/ecc library does not support the R1 curve yet")
	}
